...
def _retrieve_orders(access_token):
    ...
    response = request_with_retry(api_url, headers, network_scope="tesla")
...
def _retrieve_order_details(order_id, access_token):
    ...
    api_url = f"https://akamai-apigateway-vfx.tesla.com/tasks?{query_string}"
    response = request_with_retry(api_url, headers, network_scope="tesla")
...
def generate_share_output(detailed_orders, copy_to_clipboard=False):
    ...
    if copy_to_clipboard and HAS_PYPERCLIP:
...
def display_orders_SHARE_MODE(detailed_orders):
    share_output = generate_share_output(detailed_orders, copy_to_clipboard=True)
